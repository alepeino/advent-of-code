...
(defn- run-turn [adjustment-factor state monkey-id]
  (let [{:keys [items operation test iftrue iffalse]} (state monkey-id)]
    (reduce
      (fn [new-state item]
        (let [adjusted-item (mod item adjustment-factor)
              new-item      (operation adjusted-item)
...
(defn- run-rounds [total-rounds state]
  (let [monkey-ids        (-> state keys sort)
        adjustment-factor (->> state vals (map :test) (apply *))]
    (loop [round 0
           state state]
      (if (= total-rounds round)
        state
        (recur (inc round) (reduce (partial run-turn adjustment-factor) state monkey-ids))))))
